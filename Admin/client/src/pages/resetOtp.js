...
function ResetPasswordOtpVerification () {
  ...
  const handleOTP = (event) => {
    ...
    if (successMessage) {
      ...
      // } else 
      if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(values.emailorPhoneNumber)) {

        setSuccessMessage('');
        setIsLoading(true);
        fetch(`http://localhost:5000/api/admin/send-email`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
...
const resendOTP=()=>{
  ...
  return (
    <div className="otp_container">
      ...
      <div >
        <p className='otp_success'>{successMessage}</p>
      </div>
      ...
      <form >
        <div className='otp_password'>
          ...
          <div >
            <p className='time_remain'>Time Remaining:</p>
          </div>
